        ...
        for customer_journey in business_capability.iterdir():
            ...
            for customer_sub_journey in customer_journey.iterdir():
                ...
                for test_id in customer_sub_journey.iterdir():
                    ...
                    score_pattern =  r"Metric(?: Score)?:\s*(\d+)"
                    for result_file in result_files:
                        if 'edc' in str(result_file):
                            result_edc = re.search(score_pattern, open(result_file, 'r').read(), re.IGNORECASE)
                            result_edc = result_edc.group(1) if result_edc else 'N/A'
                            # print(result_edc)
                        if 'fiware' in str(result_file):
                            result_fiware = re.search(score_pattern, open(result_file, 'r').read(), re.IGNORECASE)
                            result_fiware = result_fiware.group(1) if result_fiware else 'N/A'
                            # print(result_fiware)
